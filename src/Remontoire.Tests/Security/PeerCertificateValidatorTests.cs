...
public class PeerCertificateValidatorTests {
    ...
    public void Accepts_a_CA_signed_certificate_with_the_expected_subject() {
        ...
        var validator = new PeerCertificateValidator(caPublic, expectedSubjects: ["CN=node-1"]);
    ...
    [Fact]
    public void Accepts_any_subject_when_no_expected_subjects_are_configured() {
        // An individual unconfigured peer, or a cluster where nobody ever set
        // ExpectedCertificateSubject at all — relies solely on the chain-build (§5.2/§5.4).
        var (caPublic, caWithKey) = DevClusterCertificateFactory.CreateCa();
        var leaf = DevClusterCertificateFactory.CreateLeaf(caWithKey, "CN=node-1");
    ...
    [Fact]
    public void Accepts_a_subject_matching_any_entry_in_a_multi_peer_union() {
        // The server side never knows, before validation, which specific peer is connecting — it
        // must accept a match against ANY configured peer's expected subject, not just the first.
        var (caPublic, caWithKey) = DevClusterCertificateFactory.CreateCa();
        var leaf = DevClusterCertificateFactory.CreateLeaf(caWithKey, "CN=node-2");
        var validator = new PeerCertificateValidator(caPublic, expectedSubjects: ["CN=node-1", "CN=node-2", "CN=node-3"]);
    ...
    public void Rejects_a_CA_signed_certificate_with_the_wrong_expected_subject() {
        ...
        var (caPublic, caWithKey) = DevClusterCertificateFactory.CreateCa();
        var leaf = DevClusterCertificateFactory.CreateLeaf(caWithKey, "CN=node-1");
        var validator = new PeerCertificateValidator(caPublic, expectedSubjects: ["CN=node-2"]);
