...
public interface IRaftLog {
    /// <summary>
    /// Index of the last entry, or <see cref="SnapshotIndex"/> when the tail is empty.
    /// </summary>
    ulong LastIndex { get; }

    /// <summary>
    /// Term of the entry at <see cref="LastIndex"/>, or <see cref="SnapshotTerm"/> when the tail is empty.
    /// </summary>
    ulong LastTerm { get; }

    /// <summary>
    /// Index of the last entry covered by the most recent snapshot; zero when none.
    /// </summary>
    ulong SnapshotIndex { get; }

    /// <summary>
    /// Term of the entry at <see cref="SnapshotIndex"/>; zero when none.
    /// </summary>
    ulong SnapshotTerm { get; }

    /// <summary>
    /// Returns the term of the entry at <paramref name="index"/> (which may equal <see cref="SnapshotIndex"/>).
    /// </summary>
    ValueTask<ulong> GetTermAtAsync(ulong index, CancellationToken cancellationToken = default);

    /// <summary>
    /// Reads entries from <paramref name="fromIndex"/> onward — used to catch up lagging followers.
    /// </summary>
