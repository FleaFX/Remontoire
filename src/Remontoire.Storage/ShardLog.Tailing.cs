...
public sealed partial class ShardLog {
    // Pure forwarder — WalWriter.ReadCommittedAsync hands off each record the moment it's
    ...
    // completes its ReadCommittedAsync stream (ShardLog.DisposeAsync).
    async Task RunTailingLoopAsync() {
        await foreach (var record in _walWriter.ReadCommittedAsync())
